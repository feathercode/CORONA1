...
void xf_filter_bworth1_d_R(double *X, size_t *n, float *sf, float *lf, float *hf, float *r, char *message) {

	char *thisfunc="xf_filter_bworth1_d_R\0";
	size_t ii;
	double *Y;
	double omega,a0,a1,a2,b1,b2,c0,c1,c2,d1,d2;
	
	size_t nn= n[0];
	float sample_freq= sf[0];
	float low_freq= lf[0];
	float high_freq= hf[0];
	float res= r[0];
	double M_PI= 3.14159265358979323846264338327950288419716939937510;
	
	sprintf(message," ");
	if(nn<4) {sprintf(message,"%s [ERROR]: no filtering - number of input samples is less then 4",thisfunc); return; }
	if(low_freq>(sample_freq/2.0)) {sprintf(message,"%s [ERROR]: low frequency %g must be <= half of sample frequency %g",thisfunc,low_freq,sample_freq); return; }
	if(high_freq>(sample_freq/2.0)) {sprintf(message,"%s [ERROR]: high frequency %g must be <= half of sample frequency %g",thisfunc,high_freq,sample_freq); return;}
	...
	if(!Y) {sprintf(message,"%s [ERROR]: insufficient memory",thisfunc);return;}
